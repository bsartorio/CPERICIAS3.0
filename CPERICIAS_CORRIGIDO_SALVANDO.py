...
class JanelaDetalhes:
    ...
    def criar_interface(self):
        ...
        for label, key, width in campos:
            ...
            if key in ['concluido', 'realizada']:
                ...
                text = tk.Text(frame, height=5, width=40, wrap=tk.WORD)
                text.insert("1.0", self.item.get(key, ""))
        ...
        self.btn_editar = ttk.Button(self.btn_frame, text="Editar", command=self.habilitar_edicao)
        self.btn_editar.pack(side=tk.LEFT, padx=5)
        ttk.Button(self.btn_frame, text="Fechar", command=self.top.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def habilitar_edicao(self):
        ...
        ttk.Button(self.btn_frame, text="Salvar", command=self.salvar_alteracoes).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.btn_frame, text="Cancelar", command=self.top.destroy).pack(side=tk.RIGHT, padx=5)

    def salvar_alteracoes(self):
        try:
            for key, widget in self.entries.items():
                if isinstance(widget, ttk.Entry):
                    self.item[key] = widget.get()
                elif isinstance(widget, tk.BooleanVar):
                    self.item[key] = widget.get()
                elif isinstance(widget, tk.Text):
                    self.item[key] = widget.get("1.0", tk.END).strip()
...
class SistemaPrazos:
    ...
    def ver_item(self):
        ...
        try:
            for i, item in enumerate(prazos.get(data_str, [])):
                if item["processo"] in texto:
                    JanelaDetalhes(
                        self.root, 
                        item, 
                        'prazo', 
                    ...
                    return
            for i, item in enumerate(pericias.get(data_str, [])):
                if item["processo"] in texto:
                    JanelaDetalhes(
                        self.root, 
                        item, 
